...
with col_input:
    ...
    # API Key dùng chung cho toàn bộ App
    api_key = st.text_input("🔑 Nhập Google Gemini API Key (Bắt buộc cho tính năng AI):", type="password", key="ai_key")
    
    # --- KHU VỰC AI TRỢ LÝ KHẢO SÁT ---
    with st.expander("🤖 AI Trợ Lý Khảo Sát Giá", expanded=False):
        ...
        if st.button("🔍 Yêu cầu AI Khảo sát", type="primary", key="btn_ai"):
            if api_key == "":
                st.warning("Vui lòng nhập API Key ở trên trước!")
            else:
...
with col_result:
    if not is_valid_schedule:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📊 P&L Chi Tiết", "🗓️ Dòng Tiền & Vay", "📉 Phân Tích Độ Nhạy", "🤖 AI Nhận Định & Chat"])
        ...
        with tab3:
            ...
            for cost_adj in variations:
                ...
                irr_matrix.append(irr_row); npat_matrix.append(npat_row)
                
            st.markdown("#### Tác động lên tỷ suất sinh lời Project IRR")
            df_sens_irr = pd.DataFrame(irr_matrix, index=row_labels, columns=col_labels)
            st.dataframe(df_sens_irr.style.format("{:.1%}", na_rep="N/A").background_gradient(cmap='RdYlGn', axis=None), use_container_width=True)
            
            st.markdown("#### Tác động lên Lợi nhuận sau thuế - NPAT (Triệu VND)")
            df_sens_npat = pd.DataFrame(npat_matrix, index=row_labels, columns=col_labels)
            st.dataframe(df_sens_npat.style.format("{:,.0f}").background_gradient(cmap='RdYlGn', axis=None), use_container_width=True)
